/*
 * Linux driver for KNJN Dragon PCIe (IORESOURCE_MEM)
 *
 * Pierre Ficheux (pierre.ficheux@igmail.com)
 *
 * 6/2014
 */
...
static int dragon_pci_mem_probe(struct pci_dev *dev, const struct pci_device_id *ent)
{
  ...
  if (mypin) {
    ...
    ret = request_irq(dev->irq, dragon_pci_mem_irq_handler, 0/*IRQF_SHARED*/, "dragon_pci_mem", data);
    if (ret < 0) {
