...
main (int ac, char **av)
{
  ...
  size[0] = DATA_SIZE;
  size[1] = 0;
  status = libusb_bulk_transfer (handle, 6, size, 2, &l, 5000);
  if(status != USB_STATUS_OK) {
    close_device_error("error bulk transfer 6", status);
  }
  else
    printf ("l= %d\n", l);

  status = libusb_bulk_transfer (handle, LIBUSB_ENDPOINT_IN | 4, buffer, DATA_SIZE, &l, 5000);
  if(status != USB_STATUS_OK) {
    close_device_error("error bulk transfer 4", status);
  }
  else
    printf ("l= %d\n", l);
